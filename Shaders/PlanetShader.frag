...
vec3 Voronoi(const vec3 pos, const float cellMin, const vec3 tangent, const vec3 bitangent)
{
  ...
  //minDistToCell /= distance(closestCell, secClosestCell);
...
vec3 TerrainColor()
{
  ...
  float scale = (slope > 0.6f) ? 100.f : 30.f;

  vec3 spherePos = normalize(inPos);
  vec3 tangent = FastTangent(spherePos);
  vec3 bitangent = cross(spherePos, tangent);

  spherePos *= scale;
  vec3 voronoiHash = Voronoi(spherePos, scale, tangent, bitangent);
  return vec3(voronoiHash.x);

  vec3 gridPos = floor(spherePos);

  float hash = voronoiHash.x;

  vec2 uv;
  uv.x = dot(spherePos - gridPos, tangent);
  uv.y = dot(spherePos - gridPos, bitangent);
...
void main()
{
  ...
  // Swapchain = vec4(VertexIdToColor(inVertId), 1.f);
